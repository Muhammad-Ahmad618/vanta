...
export const fetchAllCommentsOfATask = async (req: Request, res: Response) => {
    ...
    if (isNaN(task_idNum)) {
        return res.status(400).json({ message: "Invalid Task ID Please Try Again." })
    }
    try {
        ...
        return res.status(200).json({ message: "Comments Fetched Successfully", data: result });
    } catch (error) {
        return res.status(500).json({ message: "Error While Fetching Comments. Please Try Again" });
    }
}
...
export const AddComment = async (req: Request, res: Response) => {
    ...
    if (isNaN(task_idNum) || isNaN(user_idNum)) {
        return res.status(400).json({ message: "Invalid Task ID or User ID Please Try Again." })
    }

    if (content === "" || content === null || content === undefined) {
        return res.status(400).json({ message: "Comment Cannot Be Empty Please Try Again." })
    }

    try {

        const result = await createTaskComment(task_idNum, user_idNum, content);
        return res.status(200).json({ message: "Comment Created Successfully", data: result });
